"""Retired legacy custom server; QA uses VERL 0.6's native vLLMHttpServer.

This module intentionally installs no server or monkey patch. See config.yaml.
"""
# [修改目的] 退出训练加载路径，并使显式 import 本模块不再依赖已移除的 AsyncvLLMServer。
# 旧实现以 main 为基线完整注释保留；不要将本模块配置为 custom_async_server。
# [原代码保留]
# import ray
