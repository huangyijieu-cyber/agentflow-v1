...
class Initializer:
    ...
    def _is_in_enable_tool_list(self, tool_name):
        ## check the tool wether in enable tool list
        short_to_long = self.tool_name_mapping.get("short_to_long")
        enabled_tools = self.enabled_tools
        for tool in enabled_tools:
            if tool_name == short_to_long.get(tool):
                return True
        return False
    ...
    def run_demo_commands(self) -> List[str]:
        ...
        for tool_name, tool_data in sorted(self.toolbox_metadata.items()):
            ...
            ## whether in enable tool list
            short_to_long = self.tool_name_mapping.get("short_to_long")
            if not self._is_in_enable_tool_list(tool_name):
                print(f"X {tool_name} is not in {[ short_to_long.get(e_tool) for e_tool in self.enabled_tools]}")
                continue
...
if __name__ == "__main__":
    ...
    tool_engine = ["gpt-4o-mini", "gpt-4o-mini", "Default"]
