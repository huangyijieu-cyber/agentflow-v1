...
class Planner:
    def __init__(self, llm_engine_name: str, llm_engine_fixed_name: str,
        ...
        print("llm_engine_name in planner:", llm_engine_name)
        print("llm_engine_fixed_name in planner:", llm_engine_fixed_name)
        ...
        try:
            if isinstance(response, str):
                ...
                try:
                    ...
                    # print(f"Failed to parse response is:\n{response}")
                    print(f"Failed to parse response as JSON in Planner: {str(e)}")
            if isinstance(response, NextStep):
                print("Selection 1: parsed by NextStep.")
                context = response.context.strip()
                sub_goal = response.sub_goal.strip()
                tool_name = response.tool_name.strip()
            else:
                print("Selection 2: parsed by string with **")
